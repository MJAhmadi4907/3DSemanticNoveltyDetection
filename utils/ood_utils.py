...
# SOME CHANGES MADE TO BELOW FUNCTION FOR MORE OUTPUTS LIKE POINTCLOUD COORDINATION
@torch.no_grad()
def get_network_output(model, loader, indices=None, softmax=True):
    """ DDP impl """
    all_logits = []
    all_pred = []
    all_labels = []
    ...
    if indices is not None:
        ...
        selected_pred = all_pred[indices]
        selected_labels = all_labels[indices]
        return selected_logits, selected_pred, selected_labels, selected_points
...
@torch.no_grad()
def get_penultimate_feats(model, loader,indices= None):
    """ DDP impl """
    all_feats = []
    all_labels = []
    all_points = []
    ...
    all_points = torch.cat(all_points, dim=0)
    if indices is not None:
        selected_feats = all_feats[indices]
        selected_labels = all_labels[indices]
        selected_points = all_points[indices]
        
        return selected_feats, selected_labels, selected_points
...
def estimate_react_thres(model, loader, id_percentile=0.9):
    ...
    print(f"t = {thres:.4f}")
    print('the threshold based on the react: ',thres)
    return thres
...
#### REMEBER TO MODIFY THE INDICES FOR THE BELOW FUNCTION LATER IF NEEDED
def eval_ood_sncore(method, scores_list, preds_list=None, labels_list=None, points_list=None, src_label=1, silent=False):
    """
    ...
    # print('Tar1 Conf:', tar1_conf)
    # print('Tar1 Preds:', tar1_preds)
    # print('Tar1 Labels:', tar1_labels)
