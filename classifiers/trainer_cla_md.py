...
def get_md_react_val_loader(opt):
    ...
    if opt.corruption:
        ...
        l_corr_data = get_lrr_ist_corr_data(opt, split='test')  # list of corrupted test datasets
        assert isinstance(l_codata, list)
        assert isinstance(l_corr_data[0], data.Dataset)
...
#### ------- MJAHMADI MOST CHANGES MADE HERE ----------- #####

# EVALUATION OF OUT OF DISTRIBUTION FROM MODELNET TO SCANOBJECT
def eval_ood_md2sonn(opt, config):
    ...
    if opt.src == 'SR1':
        print("Src is SR1\n")
        id_loader = ScanObject(class_choice="sonn_2_mdSet1", **sonn_args)
        # Define the lengths of the splits
        train_size = int(0.8 * len(id_loader))  # 80% for training
        val_size = len(id_loader) - train_size   # remaining 20% for validation
        # Use random_split to create the training and validation sets
        test_dataset, val_dataset = random_split(id_loader, [train_size, val_size])       
        ...
        print("Src is SR2\n")
        id_loader = ScanObject(class_choice="sonn_2_mdSet2", **sonn_args)
        # Define the lengths of the splits
        train_size = int(0.8 * len(id_loader))  # 80% for training
        val_size = len(id_loader) - train_size   # remaining 20% for validation
        # Use random_split to create the training and validation sets
    ...
    src_logits_val, src_pred_val, src_labels_val, _ = get_network_output(model, val_loader,indices=None)
    src_logits, src_pred, src_labels, _ = get_network_output(model, test_loader,indices=None)
    tar1_logits, tar1_pred, tar1_labels, _ = get_network_output(model, ood1_loader,indices=None)
    tar2_logits, tar2_pred, tar2_labels, _ = get_network_output(model, ood2_loader,indices=None)



    # tar1_spec_logits, tar1_spec_pred, tar1_spec_labels, tar1_spec_points = get_network_output(model, ood1_loader, indices=selected_indices, softmax=True)


    # MSP
    print("\n" + "#" * 80)
    print("Computing OOD metrics with MSP normality score...")
    src_MSP_scores_test = F.softmax(src_logits, dim=1).max(1)[0]
    src_MSP_scores_val = F.softmax(src_logits_val, dim=1).max(1)[0]
    tar1_MSP_scores = F.softmax(tar1_logits, dim=1).max(1)[0]
    tar2_MSP_scores = F.softmax(tar2_logits, dim=1).max(1)[0]

    tar1_MSP_scores_spec = F.softmax(tar1_logits, dim=1).max(1)[0]
    ...
    #     return threshold
    src_MSP_scores_val = src_MSP_scores_val.cpu()   # Your list or array of inside distribution (ID) scores
    src_MSP_scores_val = np.array(src_MSP_scores_val)
    # # Confidence level
    # confidence_level = 0.8
    # threshold_inside_distribution = calculate_threshold_inside_distribution(src_MSP_scores_val, confidence_level)
    # print("threshold_inside_distribution: ", threshold_inside_distribution)

    threshold_inside_distribution = np.mean(src_MSP_scores_val)
    # Find indices where values are higher than the threshold

    tar1_MSP_scores_cpu = tar1_MSP_scores.cpu()
    indexes_above_threshold_MSP = torch.nonzero(tar1_MSP_scores_cpu > threshold_inside_distribution).flatten().numpy()
    #print('Indexes exceding the defined threshold for MSP:', indexes_above_threshold_MSP)
    tar1_spec_logits, tar1_spec_pred, tar1_spec_labels, tar1_spec_points = get_network_output(model, ood1_loader, indices=indexes_above_threshold_MSP, softmax=True)
    tar1_MSP_scores_spec = F.softmax(tar1_spec_logits, dim=1).max(1)[0]
    eval_ood_sncore(
        method = 0,
        scores_list=[src_MSP_scores_test, tar1_MSP_scores, tar2_MSP_scores],
    ...
    print('Target 1 Specs logits',tar1_spec_logits)
    print('Target 1 Specs Preds',tar1_spec_pred)
    print('Target 1 Specs labels',tar1_spec_labels)
    #print('Target 1 Specs Points',tar1_spec_points)
    print('Target 1 Specs Points length',len(tar1_spec_points))
    ...
    # tar1_spec_MLS_logits, tar1_spec_MLS_pred, tar1_spec_MLS_labels, tar1_spec_MLS_points = get_network_output(model, ood1_loader, indices=indexes_above_threshold_MSP, softmax=True)
    # print('Target 1 MLS Specs logits',tar1_spec_logits)
    # print('Target 1 MLS Specs Preds',tar1_spec_pred)
    # print('Target 1 MLS Specs labels',tar1_spec_labels)
    # print('Target 1 MLS Specs Points',tar1_spec_points)
    # print('Target 1 MLS Specs Points length',len(tar1_spec_points))

    # # entropy
    # print("\n" + "#" * 80)
    # src_entropy_scores_val = 1 / logits_entropy_loss(src_logits_val)
    # src_entropy_scores_test = 1 / logits_entropy_loss(src_logits)
    ...
    # eval_ood_sncore(
    #     scores_list=[src_entropy_scores_test, tar1_entropy_scores, tar2_entropy_scores],
    #     preds_list=[src_pred, None, None],  # computes also MSP accuracy on ID test set
    #     labels_list=[src_labels, None, None],  # computes also MSP accuracy on ID test set
    #     src_label=1)
    # print("#" * 80)
    ...
    eval_OOD_with_feats(model, train_loader, val_loader, test_loader , ood1_loader, ood2_loader, save_feats=opt.save_feats)
    ...
    output_data = {
        "src_MSP_scores_test": src_MSP_scores_test.tolist(),
        "src_MSP_scores_val": src_MSP_scores_val.tolist(),
        "tar1_MSP_scores": tar1_MSP_scores.tolist(),
        "tar2_MSP_scores": tar2_MSP_scores.tolist(),
        "tar1_spec_scores":tar1_MSP_scores_spec.tolist(),
        "tar1_spec_pred" : tar1_spec_pred.tolist(),
        "tar1_spec_labels" : tar1_spec_labels.tolist(),
    }
    ...
    with open(text_file_path, 'w') as text_file:
        for tensor in your_data:
           for row in tensor:
                text_file.write(" ".join(map(str, row.cpu().numpy())) + "\n")

    print("Data has been saved to:", text_file_path)

    return output_file_path, text_file_path # return the file path for later access



def eval_OOD_with_feats(model, train_loader, id_val_loader, src_loader, tar1_loader, tar2_loader, save_feats=None):
    ...
    val_feats, val_labels, val_points = get_penultimate_feats(model, id_val_loader,None)
    train_feats, train_labels, train_points = get_penultimate_feats(model, train_loader,None)
    src_feats, src_labels, src_points = get_penultimate_feats(model, src_loader,None)
    tar1_feats, tar1_labels, tar1_points = get_penultimate_feats(model, tar1_loader,None)
    tar2_feats, tar2_labels, tar2_points = get_penultimate_feats(model, tar2_loader,None)
    ...
    print("threshold for distance based method:", threshold)
    ...
    tar1_CE_scores_cpu = tar1_scores.cpu()
   
    indexes_above_threshold_CE = torch.nonzero(tar1_CE_scores_cpu > threshold).flatten().numpy()
    print('Indexes exceding the defined threshold for CE:', indexes_above_threshold_CE)
    #tar1_spec_logits, tar1_spec_pred, tar1_spec_labels, tar1_spec_points = get_network_output(model, ood1_loader, indices=indexes_above_threshold_MSP, softmax=True)
    ...
    output_data01 = {
              "tar1_scores": tar1_scores.tolist(),
              "tar1_pred": tar1_pred.tolist(),
              "tar1_labels": tar1_labels.tolist(),
              "tar1_distance": tar1_dist.tolist(),
              "indexes for CE":indexes_above_threshold_CE.tolist(),
    ...
    with open(text_file_path02, 'w') as text_file:
        for tensor in your_data02:
           for row in tensor:
                text_file.write(" ".join(map(str, row.cpu().numpy())) + "\n")
    
    print("Data has been saved to:", text_file_path02)
    ...
    #     src_label=1  # confidence should be higher for ID samples
    # )
    return output_file_path01, text_file_path01, text_file_path02
    print("#" * 80)
